...
#This function will set the samAccountNames of the group members
...
#This function will set the UserPrincipalNames of the group members
...
#This function will set the UserPrincipalNames and samAccountNames of the group members
Function Start-GetBoth {
    foreach ($Group in $Groups){ 
    $members = Get-ADGroupMember -Identity $Group -Server $Domain -Credential $DomainCreds -Recursive|Get-ADUser -Properties displayname, userPrincipalName, samAccountName|Sort-Object -Property displayname|Select-Object DisplayName, userPrincipalName, samAccountName
    $members | foreach{ 
      $memberUPN = $_.userPrincipalName
      $memberSAM = $_.samAccountName
      $memberlist = $memberlist + "|^" + $memberSAM + "$" + "|^" + $memberUPN + "$"
     }
    "Domain: " + $Domain | Out-File -Append $MembersTxtFile -NoClobber 
    "Group name: " + $Group | Out-File -Append $MembersTxtFile -NoClobber
    $members | Out-File -Append $MembersTxtFile -NoClobber
